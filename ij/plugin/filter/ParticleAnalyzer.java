...
public class ParticleAnalyzer implements PlugInFilter, Measurements {
	...
	public static final int SHOW_DISTRIBUTIONS = 524288;

	/** Calculate and display curvature. */
	public static final int SHOW_CURVATURE = 1048576;

	/** Calculate and display fractal dimension. */
	public static final int SHOW_FRACTAL = 2097152;
	...
	protected boolean showResults,excludeEdgeParticles,showSizeDistribution,
		...
		addToManager, inSituShow, compositeRois, showOverlay, showDistributions;
	...
	*/
	...
	public boolean showDialog() {
		...
		labels[8]="Show distributions"; states[8]=(options&SHOW_DISTRIBUTIONS)!=0;
		labels[9]="Curvature Mapping"; states[9]=(options&SHOW_CURVATURE)!=0;
		labels[10]="Fractal Dimension"; states[10]=(options&SHOW_FRACTAL)!=0;
		...
		if (gd.getNextBoolean())
			options |= SHOW_DISTRIBUTIONS; else options &= ~SHOW_DISTRIBUTIONS;
		if (gd.getNextBoolean())
			options |= SHOW_CURVATURE; else options &= ~SHOW_CURVATURE;
		if (gd.getNextBoolean())
			options |= SHOW_FRACTAL; else options &= ~SHOW_FRACTAL;
	...
	*/
	...
	public boolean analyze(ImagePlus imp, ImageProcessor ip) {
		...
		showDistributions = (options&SHOW_DISTRIBUTIONS)!=0;
	...
	protected void saveResults(ImageStatistics stats, Roi roi) {
		...
		if ((options&SHOW_CURVATURE)!=0) {
			double[] curvatures = MorphoAnalyzer.calculateCurvature(roi);
			double avgCurvature = 0;
			double varCurvature = 0;
			if (curvatures.length > 0) {
				for (double c : curvatures) avgCurvature += c;
				avgCurvature /= curvatures.length;
				for (double c : curvatures) varCurvature += (c - avgCurvature) * (c - avgCurvature);
				varCurvature /= curvatures.length;
			}
			rt.addValue("Curvature", avgCurvature);
			rt.addValue("Curv. Var.", varCurvature);
		}
		
		if ((options&SHOW_FRACTAL)!=0) {
			double fractalDim = MorphoAnalyzer.calculateFractalDimension(imp.getProcessor(), roi);
			rt.addValue("Fractal Dim.", fractalDim);
		}
	...
	void showResults() {
		...
		if (showDistributions && rt.size()>0 && lastSlice)
			showDistributions();
	}

	private void showDistributions() {
		String[] parameters = {"Area", "Perim.", "Circ.", "Eq. Diam.", "AR", "Round"};
		Distribution d = new Distribution();
		for (String param : parameters) {
			if (rt.getColumnIndex(param) != ResultsTable.COLUMN_NOT_FOUND)
				d.showDistribution(rt, param, true, 0, 0.0, 0.0);
		}
	}
