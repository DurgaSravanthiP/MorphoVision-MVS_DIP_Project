...
public class Distribution implements PlugIn, TextListener {
	...
	public void run(ResultsTable rt) {
		...
		showDistribution(rt, parameter, autoBinning, nBins, nMin, nMax);
	}

	public void showDistribution(ResultsTable rt, String parameter, boolean autoBinning, int nBins, double nMin, double nMax) {
		int count = rt.size();
		float[] data = null;
		...
		if (data==null) {
			if (!IJ.isMacro()) IJ.error("Distribution", "No available results: \""+parameter+"\"");
			return;
		}
